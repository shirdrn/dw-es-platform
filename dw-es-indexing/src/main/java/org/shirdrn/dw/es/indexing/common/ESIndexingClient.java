...
public abstract class ESIndexingClient extends RetryIndexingClient<Client, IndexRequest> {

	private static final Log LOG = LogFactory.getLog(ESIndexingClient.class);
	private final List<File> inputFiles = Lists.newArrayList();
	
	public ESIndexingClient(String index, String type, Class<? extends IndexRequestCreator> clazz) {
		...
		final BulkProcessor bulkProcessor = BulkProcessor.builder(client, new BulkProcessor.Listener() {
			...
			public void afterBulk(long id, BulkRequest req, Throwable cause) {
				for(ActionRequest<IndexRequest> ar : req.requests()) {
					...
					ESIndexingClient.this.addRetryingRequest(ir);
				}
			}
